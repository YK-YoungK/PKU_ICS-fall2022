...
# I modified the pipe-full.hcl and implemented the iaddq instruction,
# since it can reduce the amount of instructions when I need to add an 
# instant number to a register.

# The computations required for iaddq instruction are described as follows.
##### fetch #####
# 1. icode:ifun <- M1[PC]
# 2. rA:rB <- M1[PC+1]
# 3. valC <- M8[PC+2]
...
# However, since the iaddq need the value of a register, so it can cause load/use hazard.
# This was handled by the application for load interlock.
# Excluding this, to implement the iaddq instruction is similar to which I did in PartB (seq).
...
bool need_regids =
	f_icode in { IRRMOVQ, IOPQ, IPUSHQ, IPOPQ, 
			 ...
			 IIADDQ };	# iaddq instruction require a regid byte

# Does fetched instruction require a constant word?
bool need_valC =
	f_icode in { IIRMOVQ, IRMMOVQ, IMRMOVQ, IJXX, ICALL, IIADDQ };	# iaddq instruction require a constant word
...
word d_srcA = [
	...
	# iaddq instruction need valB and valC but does not need valA
];

## What register should be used as the B source?
word d_srcB = [
	D_icode in { IOPQ, IRMMOVQ, IMRMOVQ, IIADDQ  } : D_rB;
	# iaddq instruction need valB
...
word d_dstE = [
	...
	# iaddq instruction need to write valE back to rB
	D_icode in { IPUSHQ, IPOPQ, ICALL, IRET } : RRSP;
...
word aluA = [
	...
	# iaddq instrction need valC and valB for add
...
word aluB = [
	E_icode in { IRMMOVQ, IMRMOVQ, IOPQ, ICALL, 
		     IPUSHQ, IRET, IPOPQ,
			 IIADDQ } : E_valB;		# iaddq instruction need valC and valB for add
...
bool set_cc = E_icode in { IOPQ, IIADDQ } &&
	...
	# iaddq instruction need to update condition codes
...
# iaadq instruction does not need memory access
